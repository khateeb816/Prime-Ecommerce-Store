...
@section('title', $category ? 'Prime - ' . $category->name : 'Prime - Products')
@section('description', 'Browse our wide selection of ' . ($category ? $category->name : 'products') . ' at Prime. Best prices and quality guaranteed.')
@section('keywords', ($category ? $category->name : 'products') . ', Prime, online shopping')
...
@section('content')
    ...
    <section class="container py-5">
        <form action="{{ $category ? route('products.category', $category->slug) : route('products.index') }}" method="GET" id="filterForm">
            @if(request('q'))
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    <h1 class="h2 fw-bold">{{ $category ? $category->name : 'All Products' }}</h1>
                ...
                <div class="col-md-6 text-md-end">
                    <div class="d-flex align-items-center justify-content-md-end gap-3">
                        ...
                        <select class="form-select form-select-sm" id="sort" name="sort" style="width: auto;" onchange="document.getElementById('filterForm').submit()">
                            ...
                            <option value="most_searched" {{ request('sort') == 'most_searched' ? 'selected' : '' }}>Most Popular</option>
                        </select>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card shadow-sm border-0">
                        ...
                        <div class="card-body">
                                ...
                                <div class="filter-scroll" style="max-height: 200px; overflow-y: auto;">
                                    @foreach ($brands as $brand)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" id="brand_{{ $brand->id }}" name="brand[]" value="{{ $brand->slug }}" {{ in_array($brand->slug, (array)request('brand', [])) ? 'checked' : '' }} onchange="this.form.submit()">
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <a href="{{ $category ? route('products.category', $category->slug) : route('products.index') }}" class="btn btn-outline-secondary w-100">
                        ...
                        @forelse ($products as $product)
                            <div class="col-md-4 col-sm-6">
                                <x-product-card
                                    ...
                                    :discount="$product->is_on_sale ? 'SALE' : null"
                                    :image="$product->image"
