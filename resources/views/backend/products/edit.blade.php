                                ...
                                <div class="card-body">
                                    ...
                                    <div id="attributes_list" class="bg-white border rounded p-3" style="min-height: 50px;">
                                        ...
                                        @forelse($attrValues as $val)
                                            <div class="attribute-row mb-3 d-flex align-items-end gap-3 p-3 border rounded bg-light"
                                                data-id="{{ $val->attribute_id }}">
                                                <div class="flex-grow-1">
                                                    <label
                                                        class="form-label text-muted small text-uppercase fw-bold mb-1">{{ $val->attribute->name }}</label>
                                                    <input type="text" class="form-control"
                                                        name="attribute_values[{{ $val->attribute_id }}][]"
                                        ...
                                        @empty
                                            <p class="text-muted text-center mb-0" id="no_attr_msg">No attributes added yet.
                                            </p>
                                        @endforelse
                                    </div>
                                </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-check form-switch mb-3">
                                        <input class="form-check-input" type="checkbox" name="is_on_sale"
                                            id="is_on_sale" {{ $product->is_on_sale ? 'checked' : '' }}>
...
@push('scripts')
    <script>
        function fetchBrandsAndAttributes(categoryId) {
            ...
            const attributeSelect = document.getElementById("attribute_selector");
            ...
            fetch(`/admin/brandsByCategory/${categoryId}`)
                ...
                .then(brands => {
                    brands.forEach(brand => {
                        ...
                        // Auto select on edit page
                        if (String(brand.id) === String({{ $product->brand_id }})) {
            ...
            fetch(`/admin/attributesByCategory/${categoryId}`)
                ...
                .then(attributes => {
                    attributes.forEach(attribute => {
                        ...
                        option.dataset.name = attribute.name;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (categorySelect && categorySelect.value) {
                fetchBrandsAndAttributes(categorySelect.value);
            }
